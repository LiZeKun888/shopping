...
public class OrderServiceImpl implements IOrderService {
   ...
   PayInfoMapper payInfoMapper;
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        if (order==null)
        {
            return ServerResponse.createServerResponseByError("订单创创建失败");
        }
        ...
        //6：扣库存
        reduceProductStrck(orderItemList);
        //7：购物车清空一下订单
        cleanCart(cartList);
        //8：返回，OrderVo
    ...
    private OrderVO assembleOrderVO(Order order,List<OrderItem> orderItemList,Integer shippingId)
    {
        ...
        orderVO.setOrderItemVoListList(orderItemVOList);
        ...
        Const.OrderStatusEum orderStatusEum = Const.OrderStatusEum.codeOf(order.getStatus());
        if (orderStatusEum!=null) {
            orderVO.setStatusDesc(orderStatusEum.getDesc());
        }
    ...
    private void cleanCart(List<Cart> cartList)
    {
        ...
        {
            cartMapper.batchDeleteList(cartList);
        }
    }
    /**
     * 扣库存
     */
    private void reduceProductStrck(List<OrderItem> orderItemList)
    {
        if (orderItemList!=null&&orderItemList.size()>0)
        {
            for (OrderItem  orderItem:orderItemList) {
    ...
    private Order createOrder(Integer userId,Integer shippingId,BigDecimal orderTotalPrice)
    {
        ...
        order.setStatus(Const.OrderStatusEum.ORDER_UN_PAY.getCode());
        //订单金额
        order.setPayment(orderTotalPrice);
        order.setPostage(0);
        order.setPaymentType(Const.PaymentEnum.ONINE.getCode());
        //保存订单
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList)
    {
       ...
       List<OrderItem> orderItemsList = Lists.newArrayList();
        for (Cart cart:cartList) {
            OrderItem orderItem = new OrderItem();
            orderItem.setUserId(userId);
            Product product =  productMapper.selectByPrimaryKey(cart.getProductId());
            if (product == null) {
                return ServerResponse.createServerResponseByError("id"+cart.getProductId()+"的商品不存在");
            }
            if (product.getStatus()== Const.ProductStatusEnum.PRODUCT_OFFLINE.getCode())
            {
                //商品下架
        ...
        return ServerResponse.createServerResponseBySuccess(orderItemsList);
    }
    @Override
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        //3：判断订单状态并取消
        if (order.getStatus()!=Const.OrderStatusEum.ORDER_UN_PAY.getCode())
        {
            return ServerResponse.createServerResponseByError("订单不可取消");
        }
        //4：返回结果
        order.setStatus(Const.OrderStatusEum.ORDER_CANCELED.getCode());
    ...
    public ServerResponse detail(Long orderNo) {
        ...
        //3：获取orderVo
        List<OrderItem> orderItemList = orderItemMapper.findOrderItemsByOrderno(order.getOrderNo());
        OrderVO assembleOrderVO = assembleOrderVO(order, orderItemList, order.getShippingId());
        //4：返回结果
        return ServerResponse.createServerResponseBySuccess(assembleOrderVO);
    }
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        //获取支付时间
        String pament_time = map.get("gmt_payment");
        Order order = orderMapper.findOrderByOrderNo(orderNo);
        if (order==null)
        {
            return ServerResponse.createServerResponseByError("订单"+orderNo+"不是本商品的订单");
        }
        if (order.getStatus()>Const.OrderStatusEum.ORDER_PAYED.getCode()){
            //防止支付宝重复回调
        ...
        if (trade_status.equals(Const.TRADE_SUCCESS)){
            //支付成功
            //更改订单的状态，更改支付时间
            order.setStatus(Const.OrderStatusEum.ORDER_PAYED.getCode());
            order.setPaymentTime(DateUtils.strToDate(pament_time));
     ...
     */
    @Override
    public ServerResponse puery_order_pay_status(Long orderNo) {
        if (orderNo==null)
        {
            return ServerResponse.createServerResponseByError("订单号不能为空");
        }
    ...
    public ServerResponse pay(Order order) throws IOException {
         ...
         *          *             .setTimeoutExpress(timeoutExpress)2
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://39.96.77.14:8080/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
                .setGoodsDetailList(goodsDetailList);
